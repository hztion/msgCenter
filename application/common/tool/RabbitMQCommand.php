...
class RabbitMQCommand
{
    ...
    {
        if (!is_array($configs)) {
            throw new ApiException('configs is not array', 200, 40000);
        }
        if (!($configs['host'] && $configs['port'] && $configs['username'] && $configs['password'])) {
            throw new ApiException('configs is empty', 200, 40000);
        }
    ...
    private function open()
    {
        if (!$this->_conn) {
            try {
                ...
                throw new ApiException('cannot connection rabbitmq', 200, 40000);
            }
        }
    }
    ...
    private function initConnection()
    {
        if (empty($this->exchange_name) || empty($this->queue_name) || empty($this->route_key)) {
            throw new ApiException('rabbitmq exchange_name or queue_name or route_key is empty', 200, 40000);
        }
